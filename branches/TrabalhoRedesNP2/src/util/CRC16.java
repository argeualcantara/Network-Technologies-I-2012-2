...
public class CRC16 {
	...
	public static String calculate(File file) {
		...
		int crc = 0x0000;
		try {
			bytes = IOUtils.readFully(new FileInputStream(file), (int) file.length(), true);
			for (byte b : bytes) {
	...
	public static String calculate(byte[] content) {
		...
		try {
			for (int i = 0; i < content.length; i++) {
				if (i == HeaderUtils.CRC_POS_1 || i == HeaderUtils.CRC_POS_2) {
					continue;
				} else {
					byte b = content[i];
					crc = (crc >>> 8) ^ table[(crc ^ b) & 0xff];
	...
	public static void addCRC(byte[] content, String crc){
		byte[] bytes = crc.getBytes();
		content[7] = bytes[0];
		content[8] = bytes[1];
	}
	
	public static String calculate(OutputStream outputStream) {

		// byte[] bytes;
		// int crc = 0x0000;
		//
		// try {
		// bytes = IOUtils.readFully(inputStream, 2616929, true);
		// for (byte b : bytes) {
		// crc = (crc >>> 8) ^ table[(crc ^ b) & 0xff];
		// }
		// } catch (Exception e) {
